...
class TestOrderPage:
    ...
    @pytest.mark.parametrize('name, surname, adress, phone, comment', [['Артем','Мязов', 'Чкалова 111',
                                                                        '77777777777','11']])
    def test_making_order_with_header_button_successful(self, name, surname, adress, phone, comment):
        ...
        assert order_page.current_url() == URL_DZEN

        order_page.switch_to_order_tab()
        order_page.click_samocat_logo()
        main_page.wait_for_main_page_loading()
        assert main_page.current_url() == URL_MAIN_PAGE

    @pytest.mark.parametrize('name, surname, adress, phone, comment', [['Денис','Пашков', 'Пушкина 222',
                                                                        '66666666666','1337']])
    def test_making_order_with_bottom_button_successful(self, name, surname, adress, phone, comment):
        main_page = MainPage(self.driver)
        main_page.open_main_page()
